...
The official jax code is released implementation found at:
    - https://github.com/google-research/vision_transformer

Acknowledgments:
    - This code is inspired by Ross Wightman's excellent repository:
...
def ViT_L_16(**kwargs):
    """ ViT-Large (Patch 16x16) """
    model = VisionTransformer(patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
                             norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def ViT_H_14(**kwargs):
    """ ViT-Huge (Patch 14x14) """
    model = VisionTransformer(patch_size=14, embed_dim=1280, depth=32, num_heads=16, mlp_ratio=4, qkv_bias=True,
                             norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def test():
    # Test with a ViT-Base model for ImageNet default settings
    net = ViT_B_16(img_size=224, num_classes=1000)
    # Example input tensor (batch size 4, 3 channels, 224x224 image)
    y = net(torch.randn(4, 3, 224, 224))
    print(y.size()) # Should be torch.Size([4, 1000])

    # Count parameters
    pytorch_total_params = sum(p.numel() for p in net.parameters() if p.requires_grad)
    print(f"Number of trainable parameters: {pytorch_total_params / 1e6:.2f}M")
